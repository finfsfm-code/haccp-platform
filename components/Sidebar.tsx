...
const Sidebar: React.FC = () => {
    const { activePage, setActivePage, currentUser, hasPermission } = useAppContext();
    const isPlatformAdmin = currentUser?.role === 'PLATFORM_ADMIN';

    if (isPlatformAdmin) {
        const adminNavItems = [
            { id: 'Dashboard', label: 'لوحة المراقبة', icon: HomeIcon },
            { id: 'CompanyManagement', label: 'إدارة الشركات', icon: UsersIcon },
            { id: 'RegistrationRequests', label: 'طلبات التسجيل', icon: ClipboardListIcon },
            { id: 'SupportTickets', label: 'تذاكر الدعم', icon: DocumentIcon },
        ...
        return (
            <aside className="w-64 bg-[var(--primary-light)] text-white flex flex-col flex-shrink-0">
                ...
                <nav className="flex-1 px-4 py-6 space-y-2">
                    {adminNavItems.map(item => <NavItem key={item.id} item={item} activePage={activePage} setActivePage={setActivePage} />)}
                </nav>
            </aside>
        );
    }
    ...
    const navGroups = [
        ...
        {
            title: 'العمليات والإنتاج',
            items: [
                { id: 'Products', label: 'المنتجات', icon: CubeIcon, permission: hasPermission('product:view') },
                ...
                { id: 'ProductionOrders', label: 'أوامر التشغيل', icon: FactoryIcon, permission: hasPermission('production:view') },
                { id: 'SalesOrders', label: 'العملاء والمبيعات', icon: ClipboardCheckIcon, permission: hasPermission('sales:view') },
            ]
        },
        {
            title: 'خطة الهاسب',
            items: [
                ...
                { id: 'PRPs', label: 'البرامج الأولية', icon: ShieldCheckIcon, permission: hasPermission('prp:view') },
        ...
        },
        {
            ...
            items: [
                ...
                { id: 'BeneficiaryInfo', label: 'بيانات الشركة', icon: CogIcon, permission: true },
            ]
        }
    ];
